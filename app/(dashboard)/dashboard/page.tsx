  ...
  return (
    <div className="fade-in">
      <div style={{ marginBottom: 28 }}>
        <div style={{ fontSize: 24, fontFamily: "var(--font-dm-serif-display),serif", color: C.forest, marginBottom: 4 }}>
          Welcome back 👋
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 18, marginBottom: 22 }}>
        <Card style={{ display: 'flex', flexDirection: 'column', alignItems: 'center',
          ...
          <div style={{ marginTop: 10, display: 'inline-flex', alignItems: 'center', gap: 6,
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: scoreColor, fontFamily: 'var(--font-inter),sans-serif' }}>
              {health.label}
            </span>
          </div>
            ...
            {[
              ...
              <div key={m.label} style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: C.forest,
                  fontFamily: "var(--font-jetbrains-mono),monospace" }}>{m.val}</div>
                <div style={{ fontSize: 10, color: C.sage, fontFamily: 'var(--font-inter),sans-serif',
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: C.forest, fontFamily: 'var(--font-inter),sans-serif' }}>
            Recent Activity
          </div>
        </div>
        {recentTx.length === 0 ? (
                ...
                return (
                  <tr key={item.id} style={{ borderBottom: i < recentTx.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                    <td style={{ padding: '13px 0', fontSize: 14, color: C.forest,
                      fontFamily: 'var(--font-inter),sans-serif', fontWeight: 500, maxWidth: 220,
                      overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{desc}</td>
                    <td style={{ padding: '13px 0', fontSize: 14, fontFamily: "var(--font-jetbrains-mono),monospace",
                    ...
                    <td style={{ padding: '13px 0', fontSize: 13, color: C.sage,
                      fontFamily: 'var(--font-inter),sans-serif' }}>{formatShortDate(item.created_at)}</td>
                  </tr>
                )
              })}
            </tbody>
